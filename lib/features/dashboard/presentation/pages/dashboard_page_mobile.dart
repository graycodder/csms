...
class _DashboardPageMobileState extends State<DashboardPageMobile> {
  ...
  void _tryLoad({bool forced = false}) {
    ...
    if (shopState is ShopSelected && authState is AuthAuthenticated) {
      ...
      print(
        'DEBUG: DashboardPageMobile _tryLoad - Triggering LoadDashboardData for shopId: $currentShopId',
      );
      context.read<DashboardBloc>().add(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                BlocBuilder<ShopContextBloc, ShopContextState>(
                  builder: (_, s) {
                    final name = s is ShopSelected
                        ? s.selectedShop.shopName
                        : 'My Business';
                    return Text(
                      name,
                      style: TextStyle(
                ...
                Flexible(
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final isSelected =
                          shop.shopId == state.selectedShop.shopId;
      ...
      child: Container(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText:
                'Search ${term.customerLabel.toLowerCase()}s by name or number...',
    ...
    filteredCustomers.sort((a, b) {
      ...
      int getScore(CustomerEntity c) {
        ...
        if (relevantSubs.isEmpty)
          return 900; // Penultimate Group: Active profile but no sub
        ...
        if (days <= warningThreshold)
          return 100; // Middle Group: Soon (0-X days)
      ...
      itemBuilder: (context, index) {
        if (index >= filteredCustomers.length) {
          return Padding(
            ...
            child: const Center(child: LoadingOverlay(size: 24)),
          );
        }
        return CustomerCard(
