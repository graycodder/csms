...
class ShopSubscriptionRepositoryImpl implements ShopSubscriptionRepository {
  ...
  Future<Either<Failure, ShopSubscriptionEntity>> getShopSubscriptionStatus(String shopId) async {
    try {
      ...
      if (data['active'] != null && data['active']['planId'] != null) {
        ...
        final planSnapshot = await _database.ref().child('subscription_plans').child(planId).get();
        if (planSnapshot.exists) {
          final planData = Map<dynamic, dynamic>.from(planSnapshot.value as Map);
  ...
  Stream<Either<Failure, ShopSubscriptionEntity>> getShopSubscriptionStatusStream(String shopId) {
    return _database.ref().child('shop_subscriptions').child(shopId).onValue.asyncMap((event) async {
      try {
        ...
        // Lookup plan price
        if (mapData['active'] != null && mapData['active']['planId'] != null) {
        ...
        return Right<Failure, ShopSubscriptionEntity>(ShopSubscriptionModel.fromJson(mapData, shopId));
      } catch (e) {
        return Left<Failure, ShopSubscriptionEntity>(ServerFailure(e.toString()));
      }
    });
  }

  @override
  Future<Either<Failure, List<ShopSubscriptionLogEntity>>> getShopSubscriptionHistory(String shopId, String ownerId) async {
    try {
      // Query root shop_subscription_logs and filter by shopId
      final query = _database.ref().child('shop_subscription_logs')
          .orderByChild('shopId')
          .equalTo(shopId);
      ...
      if (snapshot.exists) {
        ...
        final plansSnapshot = await _database.ref().child('subscription_plans').get();
  ...
  Future<Either<Failure, void>> renewShopSubscription({
    ...
    try {
      ...
      final String logId = _database.ref().child('shop_subscription_logs').push().key ?? 
